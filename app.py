...
#         ]
#         df_ejec = pd.read_csv('datasets/test_ejec.csv')

#         # general
#         ap_vigente = 523007132456704
#         gen = df_ejec[df_ejec['mes_num'] == max(df_ejec['mes_num'].unique())]

#         a = (gen['APR. VIGENTE'].sum() / 1_000_000_000_000).round(2)
#         b = ((gen['COMPROMISO'].sum() / gen['APR. VIGENTE'].sum()) * 100).round(1)
#         c = ((gen['OBLIGACION'].sum() / gen['APR. VIGENTE'].sum() ) * 100).round(1)
...
#             with col4:
#                 st.metric("% Proyección de ejecución final", f"{round(c / df_ejec['mes'].nunique() * 12, 1)}%")

#             tab = df_ejec.pivot_table(index='mes_num',
#                                     values=['APR. VIGENTE', 'OBLIGACION', 'COMPROMISO'],
#                                     aggfunc='sum')
#             tab['OBLIGACION'] = tab['OBLIGACION'].cumsum()
#             tab['COMPROMISO'] = tab['COMPROMISO'].cumsum()
...
#             else:
#                 prov_tab = tab.div(ap_vigente)
#                 st.dataframe(prov_tab)

#                 comp_rate = (prov_tab['COMPROMISO'].diff()).mean()
#                 obl_rate = (prov_tab['OBLIGACION'].diff()).mean()
#                 st.write(comp_rate)
#                 st.write(obl_rate)
...
elif selected_option == "Anteproyecto - 2026":

    st.header("Anteproyecto 2026")

    
    df26['TOTAL_mil'] = (df26["Anteproyecto "] / 1_000_000).round(1)

    fig = px.treemap(df26, path=[px.Constant('PGN'), 'Nombre Sector', 'Entidad', 'Tipo de gasto', 'Cuenta', 'Subcuenta', 'Objeto', 'Ordinal'],
                            values='TOTAL_mil',
